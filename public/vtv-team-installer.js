...
`;
  ...
  // Step 8: Report installation to VTV platform (for tracking/repairs)
  info('Registering installation...');
  try {
    const os = require('os');
    const installData = {
    ...
    // Also report machine info as a separate system entry
    const machineData = {
      system_name: `team:${email.split('@')[0]}`,
    ...
    ok('Installation registered');
  } catch (e) {
    warn('Could not register installation (non-critical)');
  }
  console.log();
